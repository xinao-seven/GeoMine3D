    ...
    <section class="analysis-dock">
        ...
        <div class="dock-content">
            ...
            <template v-else-if="activeDock === 'borehole'">
                <div v-if="currentDetail" class="borehole-workbench">
                    <aside class="borehole-brief">
                        <span>SELECTED BOREHOLE</span><strong>{{ currentDetail.name }}</strong>
                        <div><b>{{ currentDetail.totalDepth.toFixed(2) }}</b><small>总深度 / m</small></div>
                        <div><b>{{ currentDetail.layers.length }}</b><small>地层数量</small></div>
                    </aside>
                    <BoreholeChart :borehole="currentDetail" class="borehole-chart" />
                </div>
...
<style scoped>
.analysis-dock{height:100%;display:flex;flex-direction:column;background:#101411;border-top:1px solid var(--studio-border)}.dock-tabs{height:36px;display:flex;border-bottom:1px solid var(--studio-border);padding-left:8px}.dock-tabs button{height:36px;padding:0 14px;border:0;border-right:1px solid #282e29;background:transparent;color:#707970;font-size:10px;display:flex;gap:7px;align-items:center;cursor:pointer}.dock-tabs button.active{color:#dfd9ca;background:#1a201b;box-shadow:inset 0 2px var(--studio-copper)}.dock-tabs .dock-close{margin-left:auto;border-left:1px solid #282e29}.dock-content{flex:1;min-height:0;display:flex;align-items:stretch;padding:10px 16px;gap:12px;overflow:hidden}.metric{width:180px;padding:13px 15px;border-left:2px solid #735431;background:#151a16;display:flex;flex-direction:column}.metric span{font:9px Bahnschrift;letter-spacing:.12em;color:#667068}.metric strong{font:28px Bahnschrift;color:#d8d6cc;margin:4px 0}.metric small{color:#5e665f;font-size:9px}.activity-graph{flex:1;display:flex;align-items:end;gap:4px;padding:18px;background:repeating-linear-gradient(to bottom,transparent 0 24px,#232824 25px)}.activity-graph i{flex:1;background:#4f6658;opacity:.7}.borehole-workbench{width:min(760px,100%);height:100%;display:grid;grid-template-columns:170px minmax(300px,520px);gap:14px;margin:0 auto}.borehole-brief{padding:12px;border-left:2px solid #80603a;background:#151a16;display:flex;flex-direction:column}.borehole-brief>span{color:#6d766e;font:8px Bahnschrift;letter-spacing:.13em}.borehole-brief>strong{margin:5px 0 10px;color:#d6d3c9;font-size:15px}.borehole-brief>div{display:flex;align-items:baseline;justify-content:space-between;padding:7px 0;border-top:1px solid #2b312c}.borehole-brief b{color:#bd8950;font:17px Bahnschrift}.borehole-brief small{color:#656e67;font-size:8px}.borehole-chart{min-width:0;width:100%;height:100%;min-height:0;background:#131815;border:1px solid #29302a}.dock-empty,.performance-note{margin:auto;color:#677068;font-size:11px}.data-table-head,.data-row{display:grid;grid-template-columns:2fr 1fr 1fr;width:100%;padding:8px 12px;font-size:10px}.data-table-head{color:#646d65;border-bottom:1px solid #2b312c}.data-row{color:#b9beb7;background:#151a16}.performance-note{margin:0;flex:1;display:grid;place-items:center;border:1px dashed #313832}
</style>
