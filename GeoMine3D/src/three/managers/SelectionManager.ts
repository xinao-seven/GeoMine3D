...
export class SelectionManager {
    ...
    private hoverTimerId: ReturnType<typeof setTimeout> | null = null
    private pendingHoverEvent: MouseEvent | null = null
    private lastHoverTime = 0
    private readonly HOVER_THROTTLE_MS = 200
    ...
    // 鼠标移动时执行轻量拾取，仅用于悬停提示（带 120ms 时间节流）。
    private _onMouseMove = (event: MouseEvent) => {
        ...
        this.hoverTimerId = setTimeout(() => {
            ...
            if (!e || !this.enabled || !this.hoverEnabled) return

            const now = performance.now()
            if (now - this.lastHoverTime < this.HOVER_THROTTLE_MS) return

            this.lastHoverTime = now
            const obj = this._pickObjectByEvent(e)
            const nextUuid = obj?.uuid ?? null
            if (nextUuid === (this.hoveredObject?.uuid ?? null)) return

            this.hoveredObject = obj
            this.onHover?.(obj, obj ? e : null)
        }, this.HOVER_THROTTLE_MS)
    }
