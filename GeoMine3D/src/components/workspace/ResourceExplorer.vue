        ...
        <div v-if="tab === 'scene'" class="panel-body scene-tree">
            ...
            <template v-for="group in sceneGroups" :key="group.type">
                <div class="tree-layer">
                    <button class="eye-button" :class="{ visible: layerVisible[group.type] }" @click="sceneStore.setLayerVisible(group.type, !layerVisible[group.type])"><el-icon><View /></el-icon></button>
            ...
            <template v-else>
                ...
                <section v-for="group in modelGroups" :key="group.key" class="resource-group">
                    ...
                    <div v-show="!collapsed[group.key]" class="group-body">
                        <div v-for="model in group.items" :key="model.id" class="resource-row" :class="{ 'is-loaded': modelStatus(model).loaded }">
                            <button class="resource-main" :disabled="modelStatus(model).loading || modelStatus(model).loaded" @click="loadModel(model)">
                                <span class="resource-symbol" :class="group.key" :style="symbolStyle(model)">{{ group.symbol }}</span>
                                <span class="resource-name"><strong>{{ model.name }}</strong><small>{{ subtitle(model) }}</small></span>
                ...
                <section class="resource-group">
                    ...
                    <div v-show="!collapsed.borehole" class="group-body">
                        <div class="group-toolbar">
                            ...
                            <button class="ghost-button" type="button" @click="showBoreholeList = !showBoreholeList">{{ showBoreholeList ? '收起' : '展开' }}</button>
                        </div>
                        <button class="resource-row" type="button" :disabled="!boreholes.length" @click="boreholesLoaded ? removeAllBoreholes() : loadAllBoreholes()">
                            <span class="resource-symbol borehole">B</span>
                            <span class="resource-name"><strong>全部钻孔</strong><small>{{ loadedBoreholeCount }} / {{ boreholes.length }} 个孔位已加载</small></span>
                            <span class="load-state">{{ boreholesLoaded ? '移除全部' : '加载全部' }}</span>
                        </button>
                        <template v-if="showBoreholeList || boreholeKeyword">
                            <div v-for="item in filteredBoreholes" :key="item.id" class="resource-row" :class="{ 'is-loaded': isBoreholeLoaded(item.id) }">
                                <button class="resource-main" :disabled="isBoreholeLoaded(item.id)" @click="loadBorehole(item)">
                                    <span class="resource-symbol borehole">·</span>
                                    <span class="resource-name"><strong>{{ item.name || item.code }}</strong><small>{{ item.code }} · 孔深 {{ item.total_depth }} m</small></span>
                                    <span class="load-state">{{ isBoreholeLoaded(item.id) ? '已加载' : '加载' }}</span>
                                </button>
                                <button v-if="isBoreholeLoaded(item.id)" class="remove-state" title="从场景移除" @click="removeBorehole(item.id)">移除</button>
                            </div>
...
.panel-heading { height:70px; padding:15px 16px 12px; display:flex; align-items:center; justify-content:space-between; border-bottom:1px solid var(--studio-border); }
.panel-kicker { font:9px Bahnschrift,sans-serif; letter-spacing:.18em; color:var(--studio-copper); }
.panel-heading h2 { font-size:15px; margin-top:3px; }
.icon-button { width:30px; height:30px; border:1px solid var(--studio-border); background:#191f1b; color:#9da69e; cursor:pointer; }
.panel-tabs { height:38px; display:grid; grid-template-columns:1fr 1fr; border-bottom:1px solid var(--studio-border); }
.panel-tabs button { border:0; background:transparent; color:#6f7971; font-size:12px; cursor:pointer; position:relative; }
.panel-tabs button.active { color:#e1ded2; }
.panel-tabs button.active::after { content:""; position:absolute; left:25%; right:25%; bottom:-1px; height:2px; background:var(--studio-copper); }
.panel-body { flex:1; overflow:auto; padding:12px; }

/* ---------- 场景树 ---------- */
.tree-root { height:34px; display:flex; align-items:center; gap:7px; color:#d4d4cc; border-bottom:1px solid #29302a; }
.tree-root small { margin-left:auto; color:#5f685f; font-size:9px; }
.tree-caret { color:#956b3d; }
.tree-layer { height:39px; display:flex; align-items:center; gap:8px; border-bottom:1px solid #252b26; color:#aab1aa; font-size:11px; }
.tree-layer b { margin-left:auto; color:#59625b; font:9px Bahnschrift; }
.eye-button { width:24px; border:0; background:transparent; color:#485149; cursor:pointer; }
.eye-button.visible { color:#b98a52; }
.layer-dot { width:7px; height:7px; background:#8a6740; flex:none; }
.layer-dot.borehole { background:#527b70; }
.layer-dot.workingface { background:#88614f; }
.layer-dot.roadway { background:#6b7f8c; }
.scene-children { margin:2px 0 8px 34px; border-left:1px solid #343b35; }
.scene-child { height:29px; display:flex; align-items:center; gap:7px; padding-right:6px; border-bottom:1px solid #232923; color:#8d958e; font-size:10px; }
.scene-child i { width:7px; height:7px; margin-left:-4px; border:1px solid #111; flex:none; }
.scene-child span { flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.scene-child-remove { flex:none; border:0; background:transparent; color:#7d5743; font-size:10px; cursor:pointer; }
.scene-child-remove:hover { color:#ff8a5c; }
.strata-tree { margin:4px 0 12px 34px; border-left:1px solid #343b35; }
.strata-row { display:flex; border-bottom:1px solid #252b26; }
.strata-row.active { background:#1b211c; }
.strata-row button { height:31px; display:flex; align-items:center; border:0; background:transparent; color:#7d867e; font-size:10px; cursor:pointer; }
.strata-row button:hover { color:#d5d4cc; }
.layer-eye { width:28px; justify-content:center; }
.layer-select { min-width:0; flex:1; gap:7px; text-align:left; }
.layer-select i { width:7px; height:7px; margin-left:-4px; border:1px solid #111; flex:none; }
.layer-select span { flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.layer-select small { padding-right:7px; font:8px Bahnschrift; color:#525a53; }
.layer-editor { margin:8px 0 12px; padding:10px; border:1px solid #3a413b; background:#171c18; }
.editor-heading { display:flex; justify-content:space-between; gap:8px; margin-bottom:9px; color:#c6c9c1; font-size:10px; }
.editor-heading span { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.editor-heading small { color:#6c756d; white-space:nowrap; }
.layer-editor label { display:flex; align-items:center; justify-content:space-between; margin:7px 0; color:#858e86; font-size:10px; }
.layer-editor label b { font:9px Bahnschrift; color:#bd8950; }
.scene-options { padding-top:12px; border-top:1px solid #303731; }
.scene-options label { display:flex; align-items:center; justify-content:space-between; font-size:10px; color:#818a82; margin:7px 0; }

/* ---------- 资源树 ---------- */
.source-note { margin-bottom:14px; padding:10px; border-left:2px solid #8b6337; background:#181d19; display:flex; flex-direction:column; gap:3px; }
.source-note b { font:9px Bahnschrift; color:#bc8950; letter-spacing:.09em; }
.source-note span { font-size:9px; color:#687169; }
.resource-group { margin-bottom:18px; }
.group-title { width:100%; display:flex; align-items:center; gap:7px; padding:7px 3px; border:0; border-bottom:1px solid #2b322c; background:transparent; color:#879088; font-size:11px; letter-spacing:.08em; text-align:left; cursor:pointer; }
.group-title:hover { color:#c3c8c1; }
.group-title b { margin-left:auto; color:#655d4e; font:10px Bahnschrift,sans-serif; }
.group-caret { width:10px; color:#956b3d; transition:transform .15s ease; }
.group-caret.collapsed { transform:rotate(-90deg); }
.group-dot { width:7px; height:7px; flex:none; }
.group-dot.roadway { background:#6b7f8c; }
.group-dot.workingface { background:#88614f; }
.group-dot.combined { background:#c49154; }
.group-dot.strata { background:#8a6740; }
.group-dot.borehole { background:#527b70; }
.group-label { flex:1; }
.group-body { padding-top:2px; }
.group-toolbar { display:flex; gap:6px; padding:8px 0 6px; }
.group-search { flex:1; min-width:0; height:26px; padding:0 8px; border:1px solid #303832; background:#171c18; color:#c3c8c1; font-size:10px; }
.group-search::placeholder { color:#586158; }
.ghost-button { flex:none; padding:0 9px; height:26px; border:1px solid #3a413b; background:#191f1b; color:#9aa29a; font-size:10px; cursor:pointer; }
.ghost-button:hover { color:#d7d4ca; border-color:#4d564d; }
.resource-row { width:100%; display:flex; align-items:center; border-top:1px solid #272d28; color:#bfc5bd; }
.resource-row:hover { background:#1b211d; color:#eee8db; }
.resource-row:disabled { cursor:default; opacity:.62; }
.resource-main { flex:1; min-width:0; display:flex; align-items:center; gap:10px; padding:9px 7px; border:0; background:transparent; color:inherit; text-align:left; cursor:pointer; }
.resource-main:disabled { cursor:default; }
.resource-row.is-loaded .resource-main { cursor:default; opacity:.62; }
.remove-state { flex:none; padding:9px 10px; border:0; background:transparent; color:#b06a4a; font:10px Bahnschrift,sans-serif; cursor:pointer; }
.remove-state:hover { color:#ff8a5c; }
.resource-symbol { width:25px; height:25px; flex:none; display:grid; place-items:center; border:1px solid #765a36; color:#c49154; font:10px Bahnschrift,sans-serif; }
.resource-symbol.borehole { border-color:#47645d; color:#79a395; }
.resource-symbol.roadway { border-color:#51636e; color:#8aa6b5; }
.resource-symbol.workingface { border-color:#6d4f41; color:#b08a76; }
.resource-symbol.combined { border-color:#8b6a3c; color:#d3a462; }
.resource-name { flex:1; min-width:0; display:flex; flex-direction:column; }
.resource-name strong { font-size:12px; font-weight:500; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.resource-name small { font-size:9px; color:#687169; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.load-state { font:9px Bahnschrift; color:#9a7549; white-space:nowrap; }
.empty-copy { padding:12px 6px; color:#59625b; font-size:11px; }
.local-note { min-height:210px; padding:18px; border:1px dashed #3c453e; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; gap:9px; color:#747e76; }
.local-note .el-icon { font-size:28px; color:#aa7a43; }
.local-note strong { color:#c5c7bf; }
.local-note span { font-size:11px; max-width:200px; line-height:1.6; }
.local-note button { margin-top:4px; padding:7px 11px; border:1px solid #725534; background:#1b201c; color:#c6965c; cursor:pointer; }
</style>
