    ...
    <div class="toolbox" :class="{ 'is-collapsed': collapsed }">
        ...
        <nav class="tb-rail">
            <button class="tb-rail-toggle" :title="collapsed ? '展开工具箱' : '收起工具箱'" @click="collapsed = !collapsed">
                <el-icon>
                    <component :is="collapsed ? 'DArrowLeft' : 'DArrowRight'" />
                </el-icon>
            </button>

            <button v-for="group in groups" :key="group.key" class="tb-rail-btn"
                :class="{ active: openKey === group.key && !collapsed, engaged: group.engaged }" :title="group.label"
        ...
        <section v-if="!collapsed && current" class="tb-page">
            ...
            <div class="tb-page-body">
                <template v-for="(c, i) in current.controls" :key="`${groupKey(c, i)}`">
                    <!-- 动作按钮 -->
                    <button v-if="c.kind === 'action'" class="tb-action" :class="{ primary: c.primary }"
                        :disabled="c.disabled" @click="emitTool(c.event)">
                        <el-icon v-if="c.icon">
                            <component :is="c.icon" />
                        </el-icon>
                        <span>{{ c.label }}</span>
                    ...
                    <label v-else-if="c.kind === 'switch'" class="tb-switch">
                        <span>{{ c.label }}</span>
                        <el-switch :model-value="c.value" :disabled="c.disabled" inline-prompt active-text="开"
                            inactive-text="关"
                    ...
                    <div v-else-if="c.kind === 'slider'" class="tb-slider" :class="{ disabled: c.disabled }">
                        ...
                        <el-slider :model-value="c.value" :min="c.min" :max="c.max" :step="c.step"
                            :disabled="c.disabled" :show-tooltip="false"
                    ...
                    <div v-else-if="c.kind === 'segment'" class="tb-segment" :class="{ disabled: c.disabled }">
                        ...
                        <div class="tb-seg-buttons">
                            <button v-for="o in c.options" :key="o.value" :class="{ on: c.value === o.value }"
                                :disabled="c.disabled" @click="emitTool(c.event, o.value)">{{ o.label }}</button>
                        </div>
                    </div>
...
.tb-rail-btn.active {
    color: #e2c79a;
    background: #1d2320;
    border-left-color: var(--studio-copper)
}
...
.tb-seg-buttons button.on {
    color: #dfd9ca;
    border-color: var(--studio-copper);
    background: #1d2320
}

.tb-seg-buttons button:disabled {
    opacity: .45;
    cursor: default
}
